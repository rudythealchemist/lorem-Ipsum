...
// liste for 'submit event' for form

form.addEventListener('submit', (e) => {
    e.preventDefault();//default action is to submit
    ...
    // -1
    //> 9
    // if user enters values < 0 or > 9, show one paragraph

    // if any of below values are true(isNaN is a boolean value)
    if (isNaN(value) || value <= 0 || value > 9) {
        result.innerHTML = `<p class='result'>${text[randNum]}</p>`;
    } else {
        // if #s are between 1 - 9,iterate over new array 
        let tempText = text.slice(0, value);
        // console.log(tempText);
        tempText = tempText.map((item) => {
            return `<p class="result">${item}</p>`;
        }).join('');//join method creates one large string
